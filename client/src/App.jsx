...
// Нормализуем результат ставки (для уже рассчитанных)
...
// =====================
// Data layer + realtime status (без «обновлено …»)
// =====================
...
function useRealtimeBets(){
  ...
  const [tick, setTick] = useState(0) // просто счётчик обновлений
  ...
  return { bets, setBets, connected, tick }
}

// ===== статистика по ТОЛЬКО рассчитанным =====
...
// =====================
// Mobile Home — горизонтальный блок метрик + список
// =====================
function MobileHome(){
  const { bets, connected, tick } = useRealtimeBets()
  ...
  // вспышка при апдейте + подсветка новой верхней
  useEffect(()=>{
    if(!tick) return
    setFlash(true)
  ...
  // вычисления топовых метрик по последним 15
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white flex flex-col">
      ...
      {/* Summary — компактные карточки контентной ширины */}
      ...
      {/* Список ставок (самые новые сверху) */}
...
function SummaryCard({ title, subtitle, value, tone='neutral' }){
  ...
  return (
    <div className={`w-fit max-w-[85vw] min-w-0 rounded-2xl ${toneClasses} px-3 py-2 backdrop-blur-md inline-flex flex-col`}>
      <div className="text-[11px] font-semibold leading-tight break-words">{title}</div>
      <div className="text-[9px] uppercase tracking-wide text-white/70 leading-tight break-words">{subtitle}</div>
      <div className="text-base font-semibold mt-1 break-words">{value}</div>
    </div>
  )
}
...
// =====================
// Admin page — strict auth + quick status + bulk JSON import
// =====================
function AdminPage(){
  ...
  // автологин — проверяем пароль на сервере
  useEffect(()=>{
  ...
  useEffect(()=>{
    fetch('/api/bets').then(r=>r.json()).then(d=>{
      setBets(d); setJsonText(JSON.stringify(d,null,2))
    })
  },[])
  ...
  const quickStatus = async (b, to) => {
    ...
    let patch = {}
    if (to === STATUS.WON){ patch = { status: STATUS.WON, win_value: Math.round(stake * coef), win_currency: b.win_currency || 'RUB' } }
    else if (to === STATUS.LOST){ patch = { status: STATUS.LOST, win_value: -Math.abs(stake), win_currency: b.win_currency || 'RUB' } }
    else { patch = { status: STATUS.PENDING, win_value: 0, win_currency: b.win_currency || 'RUB' } }
  ...
  function normalizeImportedBet(b){
    ...
    return { time: b.time || '', id: String(b.id || Date.now() + Math.random().toString(16).slice(2)), match: b.match || '', bet: b.bet || '', status, stake_value: stake, stake_currency: b.stake_currency || 'RUB', coef: coef, win_value: win, win_currency: b.win_currency || 'RUB' }
  }
  const importAddMerge = async ()=>{
    ...
    setImportInfo('Импорт завершён: добавлено ' + importItems.length); setImportItems([])
  }
  const importReplaceAll = async ()=>{
    ...
    if(!r.ok){ setImportInfo('Ошибка импорта (авторизация?)'); return }
    setImportInfo('Импорт выполнен (замена): записей ' + importItems.length); setImportItems([])
  }
